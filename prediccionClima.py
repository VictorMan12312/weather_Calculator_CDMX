...
def predecir_futuro(fecha_objetivo):
    ...
    # Obtener historial reciente para los lags
    df_hist = obtener_historial(now - timedelta(days=7), now)
    
    # Obtener PRONÓSTICO de variables auxiliares
...
def predecir_tiempo_real():
    ...
    # Unir historial + presente (clave)
    history = pd.concat([df_hist, df_fore[df_fore.index > df_hist.index.max()]])

    # Verificar que tenemos el timestamp actual
    if now not in history.index:
        print("No hay datos disponibles para esta hora.")
        return

    # Generar features correctamente
    feat = extraer_features(history)
    ...
    # Ajuste inteligente con dato real
    if not np.isnan(temp_now):
        final = 0.2 * temp_now + 0.8 * pred
    else:
        final = pred
